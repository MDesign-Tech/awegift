    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-4xl bg-white shadow-xl rounded-lg overflow-hidden z-10 max-h-[90vh] overflow-y-auto">
          {/* Modal Header */}
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between px-6 py-4 border-b border-gray-200 bg-gray-50 gap-4">
            <div>
  ...
  return (
    <div className="w-full min-w-0">
      ...
      {selectedOrders.length > 0 && (
        <div className="mb-4 flex items-center justify-between bg-theme-color/5 border border-theme-color/20 rounded-lg p-4">
          ...
          <table className="w-full divide-y divide-gray-300 table-fixed">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {orders.map((order) => (
                <tr
                  key={order.id}
                  className={`hover:bg-gray-50 ${selectedOrders.includes(order.id) ? "bg-theme-color/5" : ""
                    }`}
                >
                    ...
                    <div className="flex items-center space-x-2">
                      <div className="flex -space-x-1 overflow-hidden flex-shrink-0">
                        {order.items.slice(0, 2).map((item, index) => (
                          <div
                            key={index}
                            className="inline-block h-6 w-6 rounded-sm ring-2 ring-white overflow-hidden flex-shrink-0"
                          >
                            {item.thumbnail ? (
                              <img
                                src={item.thumbnail}
                              ...
                              <div className="h-6 w-6 bg-gray-300 rounded-sm flex items-center justify-center">
        ...
        {orders.map((order) => (
          <div
            ...
            className={`bg-white rounded-lg shadow border border-gray-200 p-4 ${selectedOrders.includes(order.id)
              ? "ring-2 ring-theme-color bg-theme-color/5"
              : ""
              }`}
          >
