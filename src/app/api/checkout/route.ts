...
export const POST = async (request: NextRequest) => {
  ...
  try {
    ...
    let { items, email: reqEmail, orderAddress, orderId: reqOrderId, orderAmount } = reqBody;
    email = reqEmail;
    orderId = reqOrderId;
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      cancel_url: `${process.env.NEXTAUTH_URL}/checkout${orderId ? `?orderId=${orderId}&cancelled=true` : "?cancelled=true"
        }`,
      metadata: {
        email,
    ...
    if (error?.raw?.code === "show_original_customer_untrusted") {
      // Log for admin review
      console.error("Untrusted customer payment attempt:", {
        email,
        orderId,
        timestamp: new Date().toISOString(),
        error: error.message
      ...
      return NextResponse.json({
        error: "Your account cannot be used for this payment. Please contact support or try a different payment method.",
        code: "CUSTOMER_UNTRUSTED"
      }, { status: 403 });
    }

    // Handle other Stripe errors
    if (error?.type === "StripeError" || error?.raw) {
      return NextResponse.json({
