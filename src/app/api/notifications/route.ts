...
export async function GET(request: NextRequest) {
  try {
    ...
    if (scope === 'admin') {
      ...
      notifications = await notificationService.getAccountNotifications(token.sub!, token.role as UserRole);
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate scope
    if (body.scope === 'admin' && token.role !== 'admin') {
      return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
    }
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (scope === 'admin') {
      ...
      success = await notificationService.markAllAsRead(token.sub!, token.role as UserRole);
    }
