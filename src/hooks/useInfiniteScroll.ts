...
import { ProductType, OrderData, QuotationType } from "../../type";
...
interface UseInfiniteOrdersReturn {
  orders: OrderData[];
  loading: boolean;
  loadingMore: boolean;
  hasMore: boolean;
...
interface UseInfiniteQuotesReturn {
...
export const useInfiniteOrders = (
  apiEndpoint: string = "/api/admin/orders",
  initialLimit: number = 20,
  searchQuery?: string,
  statusFilter?: string,
  paymentFilter?: string
): UseInfiniteOrdersReturn => {
  const router = useRouter();
  ...
  }, [apiEndpoint, initialLimit, searchQuery, statusFilter, paymentFilter]);

  const loadMore = useCallback(() => {
    if (!loadingMore && hasMore && !loading) {
      fetchOrders(true);
    }
  }, [fetchOrders, loadingMore, hasMore, loading]);
  ...
  useEffect(() => {
    reset();
  }, [searchQuery, statusFilter, paymentFilter, reset]);
...
export const useInfiniteQuotes = (
  apiEndpoint: string = "/api/admin/quotes",
  initialLimit: number = 20
): UseInfiniteQuotesReturn => {
  const router = useRouter();
  ...
  const fetchQuotes = useCallback(async (isLoadMore = false) => {
    try {
      if (isLoadMore) {
        setLoadingMore(true);
      } else {
  ...
  }, [fetchQuotes, loadingMore, hasMore, loading]);
